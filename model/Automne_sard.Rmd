...
data_model <- data_model %>% filter(season == "fall")
...
ggplot(study_domain_sf) + geom_sf() +
  ...
  labs(x = "", y = "", fill = "Occurence", 
       title = "Carte de présence de la sardine dans le Golfe de Gascogne par Pelgas") +
...
ggplot(study_domain_sf) + geom_sf() +
  ...
  labs(x = "", y = "", fill = "Biomasse en kg", 
       title = "Carte de densité de la sardine dans le Golfe de Gascogne par Pelgas") +
  facet_wrap( ~ year, ncol = 7, nrow = 3) +
  theme_bw()
...
saveRDS(data_model, paste(local_path,"code/data_model/sard_tot.rds", sep = ""))
...
saveRDS(res.jo, file = paste(local_path,"/code/model/result_sard_fall_894",".rds", sep = ""))
res.jo <- readRDS(paste(local_path,"/code/model/result_sard_fall_894",".rds", sep = ""))
