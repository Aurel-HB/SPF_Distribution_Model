show_presence <- function(data,world,an,espece){
  #windows()
  map <-  ggplot()+
      geom_point(data=data[data$indice==0,],aes(LONG,LAT),size = 0.5,shape = 1,colour = "palegreen3")+
      geom_point(data = data[data$indice > 0,], aes(x = LONG, y = LAT), size = 1.5, colour = "black") +
